...
class CalendarRenderer {
    ...
    renderScheduleMatrix() {
        ...
        filteredEmployees.forEach((employee, empIndex) => {
            ...
            // Calculate MOV count: compare active calendar vs saved snapshot
            movCount = this.calculateMoveCount(employee, weekDates, scheduleMap);

            // Calculate shift differences for visual highlighting
            const shiftDifferences = this.calculateShiftDifferences(employee, weekDates, scheduleMap);
    ...
    calculateMoveCount(employee, weekDates, scheduleMap) {
        ...
        weekDates.forEach((date) => {
            ...
            const snapshotSchedule = snapshot.schedules.find(s => 
                s.employeeId === employee.id && s.date === dateString
            );
        ...
        // Return differences divided by 2, rounded up
    ...
    calculateShiftDifferences(employee, weekDates, scheduleMap) {
        ...
        weekDates.forEach((date) => {
            ...
            if (activeShiftId !== snapshotShiftId) {
                if (snapshotShiftId && !activeShiftId) {
                    differences.set(`${employee.id}_${dateString}`, 'deleted');
                } else if (!snapshotShiftId && activeShiftId) {
                    differences.set(`${employee.id}_${dateString}`, 'new');
                } else {
    ...
    toggleShiftFiltering(isFiltering) {
        ...
        setTimeout(() => {
            ...
            console.log(`Found ${shiftCells.length} shift cells for filtering`);
            ...
            shiftCells.forEach(cell => {
                const isWeekend = cell.classList.contains('weekend') || cell.classList.contains('saturday');
                if (isWeekend) weekendCells++;
                else weekdayCells++;
                
                const hasDifference = cell.classList.contains('shift-deleted') || 
                                    cell.classList.contains('shift-new') || 
                                    cell.classList.contains('shift-changed');
                
                if (hasDifference) changedCells++;
                
                if (isFiltering && !hasDifference) {
                    cell.classList.add('shift-unchanged');
                } else {
                    cell.classList.remove('shift-unchanged');
                }
            });
            
            console.log(`Weekend cells: ${weekendCells}, Weekday cells: ${weekdayCells}, Changed cells: ${changedCells}`);
        }, 50);
    }
}
