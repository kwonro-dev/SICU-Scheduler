...
class UIManager {
    ...
    bindContextMenuEvents() {
        ...
        if (deleteBtn) {
            deleteBtn.addEventListener('click', () => this.workforceManager.deleteCurrentShift());
        }
        ...
        if (customShiftInput) {
            customShiftInput.addEventListener('keydown', (e) => {
                if (e.key === 'Enter') {
                    this.workforceManager.addCustomShift();
                }
            });
        }

        if (addCustomShiftBtn) {
            addCustomShiftBtn.addEventListener('click', () => this.workforceManager.addCustomShift());
        }
    ...
    populateShiftOptions() {
        ...
        const availableShifts = this.workforceManager.getAvailableShiftsForEmployee(employee);
        ...
        shiftOptionBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                this.workforceManager.assignShiftToEmployee(this.currentShiftContext.employeeId, this.currentShiftContext.date, shiftId);
                this.hideContextMenu();
            });
        });
    }
    ...
    updateGridTemplate() {
        ...
        const countColumns = this.workforceManager.filterManager.getVisibleCountColumns(); // Count columns based on visibility
        ...
        if (this.workforceManager.filterManager.isColumnVisible('weekend')) {
            gridTemplate += ' 40px 40px 40px'; // Fri, Sat, Sun
        }
        if (this.workforceManager.filterManager.isColumnVisible('vacation')) {
            gridTemplate += ' 40px'; // Vac
        }
        if (this.workforceManager.filterManager.isColumnVisible('required')) {
            gridTemplate += ' 40px'; // Req
        }

        matrixContainer.style.gridTemplateColumns = gridTemplate;
        console.log(`Updated grid template: ${gridTemplate} (${totalColumns} total columns)`);
    }
}
